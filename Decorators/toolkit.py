...
def check_dns(node, nservers, qtype):
    ...
    try:
        ...
        if qtype.lower() == 'mx':
            ...
            #_data = [str(address) for address in _resolver.query(node, qtype)]  # LC
            _data = (str(address) for address in _resolver.query(node, qtype))  # Generator
...
def check_curl(uri, record=None):
    """
    Queries an uri and return the http code or a command error message.
    Optionally, you can use a different IP address for a host name than what
    would otherwise be used by the OS.

    Examples:
        import subprocess

        print(check_curl('http://www.google.com/'))
        print(check_curl('https://www.cnn.com/', 'cnn.com:443:157.166.226.25'))
    ...
    :return: string, containing HTTP header information
    """
    if record:
        a_record = '--resolve {}'.format(record)
    else:
        a_record = ''
    command = 'curl -ILks {} {}'.format(a_record, uri)
    try:
...
def main():
    ...
    print(check_curl('https://www.cnn.com/', 'cnn.com:443:157.166.226.25'))
    print(check_curl('https://google.com'))
